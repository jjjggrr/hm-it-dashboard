export const series = {
  errorRate: {
    Unit: {
      Name: "%",
    },
    Data: {
      Hub1: [2.1, 5.3, 1.2, 0.5, 3.7, 10.1, 4.5, 2.8, 6.2, 1.9, 22.5, 3.1],
      Hub2: [4.5, 3.2, 1.8, 0.9, 12.3, 5.6, 28.1, 30.4, 2.1, 1.1, 6.7, 4.9],
      Hub3: [8.5, 12.1, 7.9, 15.3, 5.4, 29.8, 11.2, 6.7, 9.1, 4.3, 14.5, 8.8],
      Hub4: [0.1, 0.3, 0.2, 1.1, 0.5, 0.8, 2.3, 0.4, 0.6, 1.5, 0.7, 0.9],
    },
    TimeUnits: ["T", "W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 30, //30-100
  },
  MTR: {
    Unit: {
      Name: "Minutes",
    },
    Data: {
      Hub1: [
        15.2, 23.1, 8.5, 45.6, 58.1, 19.7, 33.4, 41.9, 12.3, 5.8, 29.2, 37.5,
      ],
      Hub2: [
        25.8, 11.3, 42.1, 62.5, 38.7, 55.9, 14.6, 49.2, 22.8, 31.1, 7.4, 51.3,
      ],
      Hub3: [
        10.1, 36.5, 53.2, 28.9, 47.8, 16.4, 68.9, 21.7, 4.5, 44.3, 32.6, 59.9,
      ],
      Hub4: [
        6.7, 18.2, 40.3, 27.4, 52.1, 3.9, 34.8, 13.5, 46.7, 24.6, 39.1, 57.2,
      ],
    },
    TimeUnits: ["T", "W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 60, // 60 +
  },
  CFR: {
    Unit: {
      Name: "%",
    },
    Data: {
      Hub1: [
        12.3, 25.1, 8.9, 33.4, 19.7, 5.2, 38.6, 14.8, 29.5, 22.1, 3.7, 36.9,
      ],
      Hub2: [
        28.4, 15.6, 39.8, 7.1, 21.9, 31.2, 11.5, 2.4, 26.7, 18.3, 34.5, 9.8,
      ],
      Hub3: [
        23.7, 41.2, 17.4, 30.1, 6.5, 37.8, 13.9, 24.6, 4.3, 32.8, 20.2, 10.7,
      ],
      Hub4: [
        16.2, 27.3, 1.9, 35.1, 19.4, 8.6, 29.9, 12.1, 25.8, 38.2, 5.4, 21.3,
      ],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 40, // 40-100
  },
  DownTime: {
    Unit: {
      Name: "%",
    },
    Data: {
      Hub1: [1.2, 3.4, 6.1, 0.5, 4.1, 2.3, 9.2, 1.8, 4.9, 0.9, 3.7, 2.0],
      Hub2: [4.5, 2.1, 0.3, 3.9, 1.5, 7.8, 4.8, 3.3, 2.6, 0.7, 4.2, 1.1],
      Hub3: [2.8, 12.3, 4.6, 1.4, 3.1, 0.6, 4.4, 2.9, 8.4, 3.5, 0.2, 4.0],
      Hub4: [0.8, 1.6, 3.2, 4.7, 2.2, 1.0, 3.6, 5.5, 2.7, 4.3, 1.3, 0.4],
    },
    TimeUnits: ["D", "W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 5, // 5 - 100
  },
  DeploymentFrequency: {
    Unit: {
      Name: "Deployments per Week and Team",
    },
    Data: {
      Hub1: [5.6, 12.3, 8.1, 22.4, 15.7, 4.9, 18.2, 29.3, 7.5, 11.0, 34.1, 9.8],
      Hub2: [
        10.5, 6.2, 17.8, 3.1, 25.6, 41.2, 9.4, 13.7, 5.8, 20.1, 31.5, 16.9,
      ],
      Hub3: [7.1, 19.5, 4.4, 1.8, 14.2, 28.6, 6.7, 51.3, 10.9, 23.8, 8.0, 16.1],
      Hub4: [3.3, 8.9, 4.7, 11.6, 6.1, 15.3, 2.9, 20.8, 9.2, 12.5, 5.4, 18.0],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 2.5, // 0 - 2.5
  },
  SoftwareQuality: {
    Unit: {
      Name: "Rating", //A(1) - D(4)
    },
    Data: {
      Hub1: ["A", "B", "A", "C", "B", "A", "B", "C", "A", "A", "B", "D"],
      Hub2: ["B", "A", "C", "A", "B", "A", "B", "C", "C", "A", "B", "A"],
      Hub3: ["A", "C", "B", "A", "D", "A", "B", "A", "C", "B", "A", "B"],
      Hub4: ["C", "A", "B", "A", "C", "B", "A", "D", "B", "A", "C", "A"],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "M",
    CriticalLimit: "D", // a - c is ok
  },
  LeadTimeForFeatures: {
    Unit: {
      Name: "Weeks",
    },
    Data: {
      Hub1: [3.4, 9.1, 5.6, 1.2, 11.3, 7.8, 4.1, 2.5, 6.7, 14.2, 0.9, 5.0],
      Hub2: [6.3, 2.1, 7.9, 4.5, 10.5, 1.8, 3.3, 0.4, 5.9, 7.1, 8.8, 2.7],
      Hub3: [12.1, 4.8, 1.6, 6.2, 0.7, 7.5, 15.0, 3.9, 5.3, 9.9, 2.0, 6.8],
      Hub4: [1.1, 5.2, 8.4, 3.6, 7.0, 1.9, 4.3, 6.5, 0.3, 10.1, 2.8, 4.9],
    },
    TimeUnits: ["D", "W", "M", "Y"],
    DefaultUnit: "M",
    CriticalLimit: 8, // 8+
  },
  AutomatedTestCoverage: {
    Unit: {
      Name: "%",
    },
    Data: {
      Hub1: [
        25.4, 12.1, 67.8, 98.1, 45.3, 33.7, 8.9, 76.2, 51.9, 88.0, 21.5, 49.6,
      ],
      Hub2: [
        18.7, 39.4, 72.0, 5.5, 91.3, 26.8, 55.1, 83.4, 64.7, 30.2, 47.9, 99.1,
      ],
      Hub3: [
        16.5, 44.8, 80.6, 7.3, 29.1, 58.4, 73.9, 95.2, 14.2, 36.0, 61.3, 85.7,
      ],
      Hub4: [
        20.3, 11.8, 50.1, 78.5, 93.6, 32.7, 68.2, 89.9, 42.4, 57.0, 70.8, 24.0,
      ],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "M",
    CriticalLimit: 15, // 0 - 15
  },
  BuildSuccessRate: {
    Unit: {
      Name: "%",
    },
    Data: {
      Hub1: [18.3, 5.2, 11.7, 21.4, 1.9, 15.6, 9.8, 13.1, 7.4, 19.0, 4.5, 16.2],
      Hub2: [2.7, 14.8, 8.1, 17.5, 6.3, 28.9, 10.4, 19.8, 3.6, 12.9, 35.1, 9.2],
      Hub3: [15.3, 11.1, 4.8, 18.6, 25.7, 7.0, 13.4, 1.5, 16.9, 9.5, 19.9, 5.7],
      Hub4: [8.8, 17.2, 2.1, 14.3, 6.9, 12.0, 42.0, 10.6, 18.1, 3.4, 15.8, 9.3],
    },
    TimeUnits: ["D", "W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 20, // 20-100
  },
  QADetectRate: {
    Unit: {
      Name: "Items per Week and Team",
    },
    Data: {
      Hub1: [8.1, 2.3, 4.1, 12.3, 1.8, 0.5, 6.5, 3.9, 4.8, 2.0, 0.9, 3.3],
      Hub2: [4.5, 9.4, 1.2, 15.2, 0.7, 2.9, 7.8, 3.7, 4.9, 1.1, 2.5, 3.1],
      Hub3: [5.5, 3.8, 2.2, 1.6, 11.9, 4.4, 0.3, 4.9, 17.1, 2.8, 1.4, 3.5],
      Hub4: [2.7, 6.2, 4.6, 1.3, 10.7, 3.0, 0.6, 8.9, 4.1, 2.4, 1.9, 3.4],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 5, // 5+
  },
  AverageAPIResponseTime: {
    Unit: {
      Name: "ms",
    },
    Data: {
      Hub1: [
        148.3, 210.5, 89.1, 450.7, 521.6, 178.4, 66.2, 312.9, 123.0, 488.1,
        155.5, 256.7,
      ],
      Hub2: [
        162.1, 77.8, 245.3, 411.2, 134.9, 389.0, 58.7, 599.3, 199.6, 290.4,
        495.1, 140.2,
      ],
      Hub3: [
        151.4, 18.9, 288.6, 433.0, 95.8, 167.3, 222.5, 477.7, 130.1, 345.9,
        112.4, 399.8,
      ],
      Hub4: [
        205.2, 333.6, 119.7, 466.8, 678.0, 42.1, 173.5, 267.4, 145.9, 376.2,
        83.3, 191.0,
      ],
    },
    TimeUnits: ["D", "W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 500, // 500+
  },
  AcceptanceRate: {
    Unit: {
      Name: "Stars", // 1-5
    },
    Data: {
      Hub1: [4.1, 3.8, 4.5, 2.1, 3.3, 4.9, 3.6, 4.2, 3.9, 4.7, 3.1, 4.4],
      Hub2: [3.5, 4.6, 3.2, 4.8, 3.7, 4.0, 3.4, 4.3, 4.9, 3.1, 4.5, 3.8],
      Hub3: [4.2, 3.9, 4.7, 3.3, 4.1, 1.9, 4.6, 3.5, 4.8, 3.2, 4.4, 3.0],
      Hub4: [3.7, 4.9, 3.2, 4.5, 4.0, 3.6, 4.3, 3.8, 4.1, 3.4, 4.7, 3.9],
    },
    TimeUnits: ["W", "M", "Y"],
    DefaultUnit: "W",
    CriticalLimit: 3, //1-3
  },
};
